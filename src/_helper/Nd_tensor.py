...
#             core = A_array.copy()
            for pp in range(0, ndims):
                ...
                E1, XV1 = scipy.sparse.linalg.eigsh(C*(1.0 + 0.0j), k=rank[pp])
                U2[pp] = XV1
        self.U = tuple(U2)
    ...
    def factor(self, A_array, jj, rank):
        ...
        E1, XV1 = scipy.sparse.linalg.eigsh(B1*(1.0 + 0.0j), k=rank)
#         B2 = self.unfold(A_array, jj)
#         XV1 = scipy.sparse.linalg.svds(B2, return_singular_vectors='u')[0] # only return the left singular vector
        return XV1
...
#         max_val = numpy.max(numpy.abs(nuclear.ravel()))
        ...
        recovered = self.adjoint(nuclear)
        return recovered
    def permulation1(self, A, axis):
        """
        permuate axis to the leading row
...
#                 out_str +=  'z'
                ...
                out_str += abc_str[jj]
        out_str = abc_str[axis]+out_str
        einsum_instr = in_str +  ' -> ' + out_str 
        if self.debug:
            print(nd, einsum_instr)
#         B1 = numpy.einsum(einsum_instr,A,A.conj())
        B1 = numpy.einsum(einsum_instr, A)
        return B1        
    def unfold(self, input_mat, axis):
        ...
        new_shape = ()
        accumulate_shape = 1
        for pp in range(0, len(input_mat.shape)):
            if pp == axis: # the axis will be flattened if in axes
                new_shape += (input_mat.shape[pp], )
            else:
                accumulate_shape *= input_mat.shape[pp]
        new_shape = (accumulate_shape, ) + new_shape
        permed_mat = self.permulation1(input_mat, axis)
        output_mat = numpy.reshape(permed_mat, new_shape, order='C')
        return output_mat
