...
#         else: 
    ...
    def forward(self, x):
        try:
            y = numpy.einsum(self.compute_str_forward, x, *self.F_mats,optimize=self.path)
        except:
            self.path = numpy.einsum_path(self.compute_str_forward, x, *self.F_mats,optimize='optimal')[0]
            y = numpy.einsum(self.compute_str_forward, x, *self.F_mats,optimize=self.path)
        return y
    def adjoint(self, y):
        print(self.compute_str_adj)
        x = numpy.einsum(self.compute_str_adj, y, *[self.F_mats[dimid].conj() for dimid in range(0, self.ndims)],optimize='optimal')
        x /= self.scale
