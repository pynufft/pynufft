...
#         print('tensor_sn.shape',tensor_sn.shape)
        shift = 0
        for dimid in range(0, len(Nd)):
#             print('Nd[dimid] =', Nd[dimid], dimid)
            tensor_sn[shift :shift + Nd[dimid]] = self.st['tensor_sn'][dimid][:,0].real
            shift = shift + Nd[dimid]
        self.volume['tensor_sn'] = self.thr.to_device(tensor_sn)
    ...
    def set_sense(self, coil_profile):
        if coil_profile == self.Nd + (self.batch, ):        
            self.volume['gpu_coil_profile'] = self.to_device(coil_profile)
        else:
...
#         print("Now populate the array to multi-coil")
        self.prg.cPopulate(self.batch, self.Ndprod, x, xx, local_size = None, global_size = int(self.batch * self.Ndprod) )
#         print("End of x2z")
        return xx
...
#         self.thr.copy_array(z, dest=xx, )#size = int(xx.nbytes/xx.dtype.itemsize)) #size = int(xx.nbytes/8) is a hack of error in cuda backends; 8 is the byte of numpy.complex64 
        xx = self.thr.array(z.shape, dtype = self.dtype)
        self.thr.copy_array(z, dest=xx, )#size = int(xx.nbytes/xx.dtype.itemsize)) #size = int(xx.nbytes/8) is a hack of error in cuda backends; 8 is the byte of numpy.complex64 
...
#         self.thr.synchronize()
    ...
    def y2k(self, y):
        ...
        """
#         if self.parallel_flag is 1:
#             kx = self.thr.array(self.st['Kd'] + (self.batch, ), dtype = numpy.float32).fill(0.0)
#             ky = self.thr.array(self.st['Kd'] + (self.batch, ), dtype = numpy.float32).fill(0.0)
#         else:
...
#         self.prg.cSelect2(self.batch,  self.volume['KdGPUorder'],  self.volume['NdGPUorder'],     k, xx, local_size=None, global_size=int(self.Ndprod * self.batch))
    ...
    @push_cuda_context
    def xx2z(self, xx):
        z = self.thr.array(xx.shape, dtype = self.dtype)
        self.thr.copy_array(xx, dest=z, )#size = int(xx.nbytes/xx.dtype.itemsize)) #size = int(xx.nbytes/8) is a hack of error in cuda backends; 8 is the byte of numpy.complex64 
...
#         self.thr.synchronize()
    ...
    def z2x(self, xx_in):
        ...
        self.prg.cAggregate(self.batch, self.Ndprod, xx_in, x, local_size = int(self.wavefront), global_size = int(self.batch * self.Ndprod * self.wavefront))
        return x
    ...
    def forward(self, gz):
        ...
        try:
            ...
            try:
                ...
                print('You have been warned!')
                pz = self.to_device(gz, )
#                 pz = self.thr.to_device(numpy.asarray(gz.astype(self.dtype),  order = 'C' ))
                xx = self.z2xx(pz)
            except:
                if gz.shape != self.Nd + (self.batch, ):
                    print('shape of the input = ', gz.shape, ', but it should be ', self.Nd + (self.batch, ))
                raise
    ...
    def forward_single(self, x):
        try:
            ...
            try:
                print('In x2z(): The input array may not be a GPUarray.')
                print('Automatically moving the input array to gpu, which is throttled by PCIe.')
                print('You have been warned!')
                px = self.to_device(x, )
