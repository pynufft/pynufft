...
A batch mode NUFFT instance can compute multiple NUFFTs at the same time. 
This relies on the 'batch' keyword during the plan() stage. 
If batch is a postive integer, multiple NUFFTs are enclosed 
in the instance.  
If batch is not provided (batch='None'), it regresses to the non-batch mode.   
Note the input data and the output data of different channels are usually different. 
